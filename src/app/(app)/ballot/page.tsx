...
export default function UnifiedBallotPage() {
  ...
  useEffect(() => {
    ...
    const preBallotPhases: WizardPhase[] = ['state-select', 'demographics', 'assessment', 'profile-review', 'task-1-done'];

    if (task === '2' && preBallotPhases.includes(currentPhase)) {
  ...
  /** Demographics complete → proceed to assessment */
  ...
  /** Profile review → task 1 done (validation test) */
  ...
  if (currentPhase === 'assessment') {
    // Auto-resume saved session (skip the "Welcome back" prompt)
    const hasSavedSession = savedHybridSession && savedHybridSession.answeredAxes.length > 0;

    return (
      <div className="flex flex-col h-full">
        <HybridAssessmentView
          resumeSession={hasSavedSession ? savedHybridSession : undefined}
  ...
  if (currentPhase === 'summary') {
    return (
      <div>
        ...
        {/* Task 2 complete — return to survey */}
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-6 text-center">
        ...
        <p className="text-[15px] text-gray-500 leading-relaxed max-w-xs">
          You&apos;ve completed Task 1. Please return to the survey and continue.
        </p>
      </div>
    );
  }

  if (currentPhase === 'task-2-done') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] px-6 text-center">
        <div className="w-16 h-16 rounded-full bg-brand-primary/10 flex items-center justify-center mb-5">
          <Check className="w-8 h-8 text-brand-primary" />
        ...
        <h2 className="text-xl font-bold text-gray-900 mb-2">Great work!</h2>
        <p className="text-[15px] text-gray-500 leading-relaxed max-w-xs">
          You&apos;ve completed Task 2. Please return to the survey and continue.
        </p>
      </div>
    );
  }
